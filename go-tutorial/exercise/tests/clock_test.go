...
func TestCreateClock(t *testing.T) {
	for _, tc := range timeTestCases {
		t.Run(tc.description, func(t *testing.T) {
			if actual := exercise.New(tc.h, tc.m); tc.expected != actual.String() {
...
func TestAddMinutes(t *testing.T) {
	for _, tc := range addTestCases {
		t.Run(tc.description, func(t *testing.T) {
			if actual := exercise.New(tc.h, tc.m).Add(tc.addedValue); tc.expected != actual.String() {
...
func TestSubtractMinutes(t *testing.T) {
	for _, tc := range subtractTestCases {
		t.Run(tc.description, func(t *testing.T) {
			if actual := exercise.New(tc.h, tc.m).Subtract(tc.subtractedValue); tc.expected != actual.String() {
...
func TestAddMinutesStringless(t *testing.T) {
	for _, tc := range addTestCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			expected := exercise.New(wantHour, wantMin)
			if actual := exercise.New(tc.h, tc.m).Add(tc.addedValue); !reflect.DeepEqual(actual, expected) {
...
func TestSubtractMinutesStringless(t *testing.T) {
	for _, tc := range subtractTestCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			wantMin, _ := strconv.Atoi(split[1])
			expected := exercise.New(wantHour, wantMin)
			if actual := exercise.New(tc.h, tc.m).Subtract(tc.subtractedValue); !reflect.DeepEqual(actual, expected) {
...
func TestCompareClocks(t *testing.T) {
	for _, tc := range equalTestCases {
		t.Run(tc.description, func(t *testing.T) {
			clock1 := exercise.New(tc.c1.h, tc.c1.m)
			clock2 := exercise.New(tc.c2.h, tc.c2.m)
